...
import { Sponsor, updateSponsorAction } from "@/services/sponsors";
...
export default function SponsorDetailClient({ sponsor }: Props) {
  const router = useRouter();
  const [isEditing, setIsEditing] = useState(false);

  // Pre-bind the action with the sponsor ID
  const updateActionWithId = updateSponsorAction.bind(null, sponsor.id);
      ...
      <div className="relative bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-6 pb-6">
          ...
          <div className="-mt-10 mb-4 flex items-end gap-4">
            ...
            <div className="pb-1">
              <StatusBadge status={sponsor.is_active ? "active" : "inactive"} />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
              {isEditing ? (
                <input
                  name="full_name"
                  defaultValue={sponsor.full_name}
                  required
                  className="text-2xl font-bold text-gray-800 bg-transparent border-b-2 border-primary-700 focus:outline-none w-full"
                />
              ) : (
                <h2 className="text-2xl font-bold text-gray-800">{sponsor.full_name}</h2>
              )}
              <p className="mt-0.5 text-sm text-gray-500 font-mono">#NTT{sponsor.id}</p>
            </div>

            <div className="flex gap-3 flex-wrap text-xs text-gray-500">
              <span className="inline-flex items-center gap-1 bg-gray-100 rounded-full px-3 py-1">
                Giới tính: {sponsor.gender}
              </span>
              <span
                className={`inline-flex items-center gap-1 rounded-full px-3 py-1 font-semibold ${sponsor.is_active ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-600"
                  }`}
              >
                {sponsor.is_active ? "● Hoạt động" : "● Không hoạt động"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <SectionCard icon={Link2} title="Liên kết tài khoản & Trạng thái">
            ...
            <InfoRow
              ...
              isEditing={isEditing}
              value={sponsor.is_active ? "Hoạt động" : "Tạm dừng"}
              editNode={
                <div className="flex items-center pt-1.5">
                  ...
                  />
                  <span className="ml-2 text-sm text-gray-700">Kích hoạt nhà tài trợ</span>
                </div>
              }
            />
          </SectionCard>
            ...
            <InfoRow
              ...
              value={
                <span className="inline-flex items-center gap-1 text-gray-600">
                  <Clock size={13} className="text-gray-400" /> {sponsor.creator || "—"}
                </span>
              }
            />
            <InfoRow
              label="Ngày tạo"
              isEditing={false}
              value={fmtDateTime(sponsor.created_at || "")}
            />
            <InfoRow
              label="Cập nhật bởi"
              isEditing={false}
              value={
                <span className="inline-flex items-center gap-1 text-gray-600">
                  <Clock size={13} className="text-gray-400" /> {sponsor.updater || "—"}
                </span>
              }
            />
            <InfoRow
              label="Ngày cập nhật"
              isEditing={false}
              value={fmtDateTime(sponsor.updated_at || "")}
            />
          </SectionCard>
        </div>
      </div>
